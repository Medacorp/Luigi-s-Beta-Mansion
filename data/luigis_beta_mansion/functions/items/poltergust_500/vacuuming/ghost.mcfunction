...
execute rotated ~ 0 positioned ^ ^-0.5 ^1.2 if entity @e[type=minecraft:marker,tag=position,distance=..1.1,limit=1] if entity @s[scores={HurtTime=30..,Health=1..},tag=!dialog] at @s run function luigis_beta_mansion:items/poltergust_500/vacuuming/hurt_ghost
execute rotated ~ 0 positioned ^ ^-0.5 ^1.2 if entity @e[type=minecraft:marker,tag=position,distance=..1.1,limit=1] run tag @a[tag=me,limit=1] add is_pulling
