...
execute as @e[tag=!spectator,x=712,y=10,z=57,dx=8,dy=8,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 33
...
execute if entity @a[tag=same_room,limit=1] run function luigis_beta_mansion:room/original/missingno_17/ghosts
...
execute if entity @a[gamemode=!spectator,scores={Room=33},limit=1] if data storage luigis_mansion:data current_state.current_data.rooms.missingno_17{cleared:0b} store result storage luigis_mansion:data current_state.current_data.rooms.missingno_17.time_spent_in int 1 run scoreboard players add #temp Time 1
