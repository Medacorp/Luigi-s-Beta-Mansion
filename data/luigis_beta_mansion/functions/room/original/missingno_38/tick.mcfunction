...
execute as @a[gamemode=!spectator,x=672,y=120,z=38,dx=10,dy=8,dz=7] unless entity @s[scores={Room=54}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=672,y=120,z=38,dx=10,dy=8,dz=7] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 54
