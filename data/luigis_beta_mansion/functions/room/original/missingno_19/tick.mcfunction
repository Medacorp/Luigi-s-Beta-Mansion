...
execute as @e[tag=!spectator,x=736,y=10,z=29,dx=14,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 35
...
execute if entity @a[tag=same_room,limit=1] run function luigis_beta_mansion:room/original/missingno_19/ghosts
...
execute if entity @a[gamemode=!spectator,scores={Room=35},limit=1] if data storage luigis_mansion:data current_state.current_data.rooms.missingno_19{cleared:0b} store result storage luigis_mansion:data current_state.current_data.rooms.missingno_19.time_spent_in int 1 run scoreboard players add #temp Time 1
