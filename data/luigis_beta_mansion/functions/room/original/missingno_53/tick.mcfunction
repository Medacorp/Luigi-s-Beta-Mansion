...
execute as @e[tag=!spectator,x=721,y=1,z=-6,dx=21,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 69
...
execute if entity @a[gamemode=!spectator,scores={Room=69},limit=1] run function luigis_beta_mansion:room/original/missingno_53/ghosts
