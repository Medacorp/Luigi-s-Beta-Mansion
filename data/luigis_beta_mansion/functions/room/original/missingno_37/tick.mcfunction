...
execute as @a[gamemode=!spectator,x=653,y=119,z=-42,dx=30,dy=24,dz=78] unless entity @s[scores={Room=53}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=653,y=119,z=-42,dx=30,dy=24,dz=78] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 53
