...
execute as @e[tag=!spectator,x=640,y=16,z=-83,dx=10,dy=11,dz=10] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 46
...
execute if entity @a[gamemode=!spectator,scores={Room=46},limit=1] run function luigis_beta_mansion:room/original/missingno_30/ghosts
