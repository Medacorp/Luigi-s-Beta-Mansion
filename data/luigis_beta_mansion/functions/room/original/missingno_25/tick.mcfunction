...
execute as @e[tag=!spectator,x=637,y=19,z=-62,dx=12,dy=8,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 41
...
execute if entity @a[tag=same_room,limit=1] run function luigis_beta_mansion:room/original/missingno_25/ghosts
...
execute if entity @a[gamemode=!spectator,scores={Room=41},limit=1] if data storage luigis_mansion:data current_state.current_data.rooms.missingno_25{cleared:0b} store result storage luigis_mansion:data current_state.current_data.rooms.missingno_25.time_spent_in int 1 run scoreboard players add #temp Time 1
