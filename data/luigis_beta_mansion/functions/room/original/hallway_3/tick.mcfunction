...
execute as @e[tag=!spectator,x=713,y=10,z=-35,dx=7,dy=8,dz=60] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 8
...
execute if entity @a[gamemode=!spectator,scores={Room=8},limit=1] run function luigis_beta_mansion:room/original/hallway_3/ghosts
