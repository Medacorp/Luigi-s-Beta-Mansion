...
execute as @e[tag=!spectator,x=734,y=19,z=-39,dx=14,dy=8,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 70
...
execute if entity @a[gamemode=!spectator,scores={Room=70},limit=1] run function luigis_beta_mansion:room/original/missingno_54/ghosts
