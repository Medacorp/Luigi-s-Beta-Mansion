...
execute as @e[tag=!spectator,x=676,y=19,z=55,dx=12,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 18
...
execute if entity @a[gamemode=!spectator,scores={Room=18},limit=1] run function luigis_beta_mansion:room/original/missingno_2/ghosts
...
execute if entity @a[gamemode=!spectator,scores={Room=18},limit=1] if data storage luigis_mansion:data current_state.current_data.rooms.missingno_2{cleared:0b} store result storage luigis_mansion:data current_state.current_data.rooms.missingno_2.time_spent_in int 1 run scoreboard players add #temp Time 1
