...
execute if entity @a[tag=same_room,limit=1] run function luigis_beta_mansion:room/original/missingno_52/ghosts
...
execute if entity @a[gamemode=!spectator,scores={Room=68},limit=1] if data storage luigis_mansion:data current_state.current_data.rooms.missingno_52{cleared:0b} store result storage luigis_mansion:data current_state.current_data.rooms.missingno_52.time_spent_in int 1 run scoreboard players add #temp Time 1
