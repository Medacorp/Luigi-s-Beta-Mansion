...
execute as @e[tag=!spectator,x=644,y=101,z=38,dx=27,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 37
...
execute if entity @a[gamemode=!spectator,scores={Room=37},limit=1] run function luigis_beta_mansion:room/original/missingno_21/ghosts
