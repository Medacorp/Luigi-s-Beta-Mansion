...
execute at @s[scores={ActionTime=40..60}] run effect give @a[distance=..2,tag=!spectator] minecraft:instant_damage 1 0 true
execute at @s[scores={ActionTime=40..60}] run scoreboard players set @a[distance=..2,tag=!spectator] ForcedDamage 4
execute at @s[scores={ActionTime=40..60}] if entity @a[distance=..2,tag=!spectator] run tag @s add laugh
